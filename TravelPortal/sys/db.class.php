...
class Mysql
{
    ...
    public final function connect($host, $db, $user, $pass)
    {
        $this->host = $host;
        $this->db = $db;
        $this->user = $user;
        ...
        $this->connection = mysqli_connect($host, $user, $pass) or trigger_error(mysql_error());
        mysqli_select_db($this->db, $this->connection) or trigger_error(mysqli_error());
    }
    ...
    public final function update($tbl, $cols, $vals, $condition = null)
    {
        if (!$tbl or !$cols or !$vals) {
            ...
            $sql = "update $tbl set " . join(' , ', $qr) . $condition;
